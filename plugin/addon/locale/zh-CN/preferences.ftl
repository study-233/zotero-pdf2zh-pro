...
pref-glossary-title = 术语表与定向校对
pref-glossary-hint = 导入 UTF-8 CSV，必需列为 source,target，tgt_lng 可省略或留空。重新导入会替换当前术语表；格式或译法冲突时保留原表。
pref-glossary-import = 导入 / 替换 CSV
pref-glossary-clear = 清除术语表
pref-semanticReview = 启用正文定向校对
pref-semanticReview-hint = 默认关闭，可手动开启。针对可能影响正文含义的段落追加校对，会增加翻译时间和 API 用量。重试和补译沿用原任务设置。
