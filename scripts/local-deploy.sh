...
profile and the Homebrew-managed zotero-pdf2zh-next service.
...
expected = sys.argv[1]
actual = importlib.metadata.version("zotero-pdf2zh-next")
if actual != expected:
...
BREW_SERVICE="zotero-pdf2zh-next"
...
ADDON_ID="zotero-pdf2zh-next@nightwatcher.local"
...
PLUGIN_XPI_SOURCE="$REPO_ROOT/plugin/build/zotero-pdf2zh-next.xpi"
...
cp -p "$PLUGIN_XPI_SOURCE" "$ARTIFACT_DIR/zotero-pdf2zh-next.xpi"
PLUGIN_XPI_SOURCE="$ARTIFACT_DIR/zotero-pdf2zh-next.xpi"
...
cp -p "$PLUGIN_XPI_SOURCE" "$DEPLOY_DIR/new/zotero-pdf2zh-next.xpi"
...
OLD_XPI="$DEPLOY_DIR/old/zotero-pdf2zh-next.xpi"
OLD_WHEEL=""
...
atomic_install_xpi "$DEPLOY_DIR/new/zotero-pdf2zh-next.xpi"
[[ "$(sha256_file "$XPI_PATH")" == "$(sha256_file "$DEPLOY_DIR/new/zotero-pdf2zh-next.xpi")" ]] ||
