...
use std::{env, fs, io::Write, path::PathBuf, process::Command, time::{SystemTime, UNIX_EPOCH}};
...
fn valid_version(value: &str) -> bool {
    ...
    let numbers: Option<Vec<u32>> = parts.iter().map(|part| {
        if part.is_empty() || !part.bytes().all(|c| c.is_ascii_digit()) { None }
        else { part.parse::<u32>().ok().filter(|number| *number <= i32::MAX as u32) }
    }).collect();
...
pub fn available() -> bool {
    [
        (HKEY_CURRENT_USER, format!(r"Software\Microsoft\EdgeUpdate\Clients\{CLIENT_ID}")),
        (HKEY_LOCAL_MACHINE, format!(r"SOFTWARE\WOW6432Node\Microsoft\EdgeUpdate\Clients\{CLIENT_ID}")),
        (HKEY_LOCAL_MACHINE, format!(r"SOFTWARE\Microsoft\EdgeUpdate\Clients\{CLIENT_ID}")),
    ].iter().any(|(root, path)| {
        RegKey::predef(*root).open_subkey(path).ok()
...
fn log_path() -> PathBuf {
    crate::ProductPaths::discover().map(|paths| paths.logs_dir.join("webview2-setup.log"))
...
fn prepare() -> Result<bool, String> {
    ...
    let result = (|| {
        ...
        let powershell = PathBuf::from(system_root).join(r"System32\WindowsPowerShell\v1.0\powershell.exe");
        let status = Command::new(powershell)
            .args(["-NoProfile", "-STA", "-ExecutionPolicy", "Bypass", "-File"])
            .arg(&script).arg("-LogPath").arg(log_path())
            .creation_flags(0x0800_0000) // CREATE_NO_WINDOW; the WinForms window remains visible.
            .status().map_err(|e| format!("无法启动环境准备程序：{e}"))?;
        match status.code() {
...
pub fn ensure_ready(autostart: bool) -> bool {
    ...
    match prepare() {
        ...
        Err(error) => {
            log(&error);
            let message = format!("无法准备 Microsoft Edge WebView2 运行环境。\n{error}\n\n日志：{}", log_path().display());
...
mod tests {
    ...
    fn rejects_missing_zero_and_malformed_registrations() {
        for value in ["", " ", "0.0.0.0", "not-a-version", "123", "1..2", "-1.0", "1.2.3.4.5", "2147483648.0"] {
